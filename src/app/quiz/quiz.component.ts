...
export class QuizComponent implements OnInit {
  ...
  selectQuestion(query:number) {
    if (query === this.quizs[this.index].correct){
      this.correctAnswer++
    }
  }

  nestQuestion() {
    if(this.index <= this.quizs.length - 2){
      this.index++
    }else{
      console.log('number of win question' , this.correctAnswer)
    }
  }
}
